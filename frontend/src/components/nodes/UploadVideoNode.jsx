...
export default function UploadVideoNode({ id, data, selected }) {
  ...
  return (
    <NodeContainer title="Upload Video" icon={Video} isSelected={selected} nodeId={id}>
        ...
        {!data.url ? (
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:bg-indigo-50 hover:border-indigo-300 transition-colors nodrag flex flex-col items-center justify-center cursor-pointer relative h-24">
            ...
            <span className="text-xs text-gray-500 font-medium">
              {isUploading ? 'Uploading...' : 'Click to Pick Video'}
            </span>
            <input 
              type="file" 
              accept="video/*" 
          ...
          <div className="relative group w-full">
            <video src={data.url} controls className="w-full h-32 object-cover rounded-md border border-gray-200 block" />
            <button 
              onClick={() => updateNodeData(id, { url: null })}
              className="absolute top-1 right-1 bg-white p-1 rounded-md shadow text-xs text-red-500 opacity-0 group-hover:opacity-100 transition-opacity nodrag cursor-pointer select-none z-10"
            >
              Clear
            </button>
          </div>
